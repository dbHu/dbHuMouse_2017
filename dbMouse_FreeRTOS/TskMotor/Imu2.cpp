...
void uDmaFillCtrlStruct()
{
    ...
    *(uint32_t *)&(pui8ControlTable[0x1c4]) = (uint32_t )&imuData + sizeof(imuData) - 1;
     ...
     */
    *(uint32_t *)&(pui8ControlTable[0x1c8]) = 1 | (0<<3) | (13<<4) | (2<<14) | (0<<18) | (0<<24) | (3<<26) | (0<<28) | (0<<30);
}

void i2cRxISR(void)
{
    HWREG(I2C7_BASE | I2C_O_MICR) = 0x1;
    HWREG(I2C7_BASE | I2C_O_MIMR) &= ~0x1;

    //I2C_O_FIFOCTL use reset value
    HWREG(I2C7_BASE | I2C_O_FIFOCTL) |= 0x60040004;

    HWREG(I2C7_BASE | I2C_O_MBLEN) = 14;
...
{
    //I2C_O_FIFOCTL use reset value
    HWREG(I2C7_BASE | I2C_O_FIFOCTL) |= 0x00044004;
    //set slave address & transmits
    ...
    HWREG(I2C7_BASE | I2C_O_MICR) = 0x00000001;
...
int8_t i2cInitConfig(unsigned char *seq, int8_t len)
{
    ...
    //I2C_O_FIFOCTL, flush and trig level 4
    HWREG(I2C7_BASE | I2C_O_FIFOCTL) = 0x40044004;
...
//    HWREG(I2C7_BASE | I2C_O_MDR) = 0x55;
    HWREG(I2C7_BASE | I2C_O_MCS) = I2C_MASTER_CMD_FIFO_SINGLE_SEND;
//    HWREG(I2C7_BASE | I2C_O_MCS) = 0x07;
...
void ImuInit(void)
{
    ...
    HWREG(SYSCTL_SRI2C) |= 1 << 7;
    HWREG(SYSCTL_SRI2C) &= ~(1 << 7);
    while(HWREG(SYSCTL_SRI2C) & (1 << 7) == 0)
    {
    }
    ...
    // OPEN DRAIN
    HWREG(GPIO_PORTD_AHB_BASE | GPIO_O_ODR) |= 0x2;
    // port D[1..0] as AF2(i2c)
    HWREG(GPIO_PORTD_AHB_BASE | GPIO_O_PCTL) &= 0xFFFFFF00;
    HWREG(GPIO_PORTD_AHB_BASE | GPIO_O_PCTL) |= 0x00000022;
    ...
    //    //I2C_O_FIFOCTL use reset value
    HWREG(I2C7_BASE | I2C_O_FIFOCTL) |= 0x40004000;

//    HWREG(I2C7_BASE | I2C_O_MCS) &= 0;
//
//    // len = 0
